...
/* Note we have to make sure not to include headers twice.
   ...
   PEI_HEADERS to prevent double including in coff-aarch64.c  */
#define PEI_HEADERS
#include "sysdep.h"
#include "bfd.h"
#include "libbfd.h"
...
static void
peaarch64_get_runtime_function (bfd *abfd, struct pex64_runtime_function *rf,
			    const void *data)
{
...
static bfd_boolean
peaarch64_get_unwind_info (bfd *abfd, struct pex64_unwind_info *ui,
...
static void
peaarch64_xdata_print_uwd_codes (FILE *file, bfd *abfd,
...
static asection *
peaarch64_get_section_by_rva (bfd *abfd, bfd_vma addr, const char *sec_name)
{
...
static void
peaarch64_dump_xdata (FILE *file, bfd *abfd,
		  ...
		  struct pex64_runtime_function *rf)
{
  ...
  if (! peaarch64_get_unwind_info (abfd, &ui, xdata + addr, xdata + end_addr))
    {
      fprintf (file, _("warning: xdata section corrupt\n"));
      return;
    }
  ...
  if (ui.CountOfCodes * 2 + ui.rawUnwindCodes > xdata + xdata_section->size)
    ...
    peaarch64_xdata_print_uwd_codes (file, abfd, &ui, rf);
...
static bfd_boolean
peaarch64_bfd_print_pdata_section (bfd *abfd, void *vfile, asection *pdata_section)
{
  ...
  for (i = 0; i < stop; i += onaline)
	...
	break;
      ...
      peaarch64_get_runtime_function (abfd, &rf, &pdata[i]);
  ...
  if (strcmp (pdata_section->name, ".pdata") != 0)
    {
      ...
      if (len > 1)
	xdata_name [1] = 'x';
      xdata_section = peaarch64_get_section_by_rva (abfd, xdata_base,
						xdata_name);
      free (xdata_name);
    }
  /* Second, try the .xdata section itself.  */
  if (!xdata_section)
    xdata_section = peaarch64_get_section_by_rva (abfd, xdata_base, ".xdata");
  /* Otherwise, if xdata_base is non zero, search also inside
     other standard sections.  */
  if (!xdata_section && xdata_base)
    xdata_section = peaarch64_get_section_by_rva (abfd, xdata_base, ".rdata");
  if (!xdata_section && xdata_base)
    xdata_section = peaarch64_get_section_by_rva (abfd, xdata_base, ".data");
  if (!xdata_section && xdata_base)
    xdata_section = peaarch64_get_section_by_rva (abfd, xdata_base, ".pdata");
  if (!xdata_section && xdata_base)
    xdata_section = peaarch64_get_section_by_rva (abfd, xdata_base, ".text");
      ...
      if (rf.rva_UnwindData != 0 || virt_size_is_zero)
	{
	  if (PEX64_IS_RUNTIME_FUNCTION_CHAINED (&rf))
		...
		p = NULL;
	      ...
	      peaarch64_dump_xdata (file, abfd, xdata_section, xdata, p, &rf);
	    }
	}
    }
...
/* Functionn prototype.  */
bfd_boolean peaarch64_bfd_print_pdata (bfd *, void *);

/* Helper function for bfd_map_over_section.  */
static void
peaarch64_print_all_pdata_sections (bfd *abfd, asection *pdata, void *obj)
{
  if (CONST_STRNEQ (pdata->name, ".pdata"))
    {
      if (peaarch64_bfd_print_pdata_section (abfd, obj, pdata))
	pdata_count++;
    }
}

bfd_boolean
peaarch64_bfd_print_pdata (bfd *abfd, void *vfile)
{
  asection *pdata_section = bfd_get_section_by_name (abfd, ".pdata");

  if (pdata_section)
    return peaarch64_bfd_print_pdata_section (abfd, vfile, pdata_section);
