...
#define BADMAG(x) AARCH64BADMAG(x)

#ifdef COFF_WITH_peaarch64
# undef  AOUTSZ
...
static bfd_boolean
coff_aarch64_is_local_label_name (bfd *abfd, const char *name)
{
  if (name[0] == 'L')
    return TRUE;
...
#ifdef PE
#define aarch64coff_object_p pe_bfd_object_p
#else
#define aarch64coff_object_p coff_object_p
#endif
